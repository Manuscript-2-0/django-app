...
class CreateListTickets(TestCase):
    ...
    def test_list_tickets_should_return_list_of_tickets_on_unauthenticated_request(self):
        ...
        response = APIClient().get('/api/tickets/')
    ...
    def test_create_ticket_should_return_403_when_user_is_not_authenticated(self):
        response = APIClient().post('/api/tickets/', {
            'event': self.event.id,
        }, format='json')
        content = json.loads(response.content)
...
class RetrieveUpdateDeleteEventTest(TestCase):
    ...
    def test_get_ticket_should_return_ticket_when_user_is_not_authenticated(self):
        response = APIClient().get(f'/api/tickets/{self.ticket.id}')
        self.assertEqual(response.status_code, 200)
        content = json.loads(response.content)
        self.assertEqual(content['event_details']['id'], self.event.id)
        self.assertEqual(content['user'], self.user.id)
    ...
    def test_patch_ticket_should_return_403_when_user_is_not_authenticated(self):
        response = APIClient().patch(f'/api/tickets/{self.ticket.id}', {
            "status": "activated",
        })
        content = json.loads(response.content)
        self.assertEqual(response.status_code, 403)
    ...
    def test_delete_ticket_should_return_403_when_user_is_not_authenticated(self):
        response = APIClient().delete(f'/api/tickets/{self.ticket.id}')
        content = json.loads(response.content)
        self.assertEqual(response.status_code, 403)
        self.assertEqual(
            content['detail'], 'Authentication credentials were not provided.')
